...
- `rj_surround` captures that **single** virtual output and **slices** it across the 3 physical monitors (3 full-screen borderless windows).
...
- `rj_surround` performance/pacing is stable at **7680x1440 @ 120 FPS** (Desktop Duplication path), with instrumentation (wait/capture/render/present).
- Added **Ctrl+Alt+T** test-pattern mode in `rj_surround` (`backend=TEST`) to validate slicing/pacing without capture/IDD.
- Verified the 3-monitor slice output looks correct/seamless when driven by the synthetic 7680x1440 source.
- Implemented Option B logic in `rj_surround`: detect a single wide monitor when present and capture it via Desktop Duplication; otherwise fall back to 3-monitor DD composite.
...
- CMake updated to use `src/rj_surround.cpp`.
- Documentation moved under `doc/`.

### In progress

- Pivot `rj_surround` capture selection to prefer the **IDD virtual output** (single 7680x1440) and slice directly.
...
4) `rj_surround`: capture the virtual output
...
- For games that only render to the primary display, the intended approach is to set the IDD virtual display as **Main display** (with the caveat that the primary desktop may be “invisible” if `rj_surround` is not running to present it).
...
- Once the IDD driver is installed and Windows exposes the virtual `7680x1440@120` display, the core runtime loop becomes:
  ...
  - `rj_surround` captures that single wide output
  - `rj_surround` slices to the 3 physical monitors
...
- Run `rj_surround` and ensure it selects the wide output (`backend=DD ddmode=single_wide`).
